...
def creer_filtre_passe_haut(cutoff, fs, ordre=4):
    
    """Crée un filtre passe-haut Butterworth"""
    nyquist = 0.5 * fs
    return butter(ordre, cutoff / nyquist, btype='high', analog=False)
...
def afficher_spectres_et_bode(signal, signal_filtre, b, a, fs):
    ...
    plt.plot(freqs, spectre_filtre, label='Signal Filtré (Passe-Haut)', color='red', alpha=0.7)
...
def filtrer_audio(fichier_entree, cutoff):
    ...
    """Applique un filtre passe-haut et affiche les résultats"""
    ...
    # Créer et appliquer le filtre passe-haut
    b, a = creer_filtre_passe_haut(cutoff, fs)
